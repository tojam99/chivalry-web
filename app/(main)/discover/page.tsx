  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative bg-cream-300 rounded-3xl overflow-hidden aspect-[3/4] max-h-[520px]">
          ...
          {photoIndex > 0 && (
            <button
              ...
              className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-black/40 transition-colors"
            >
          ...
          {photoIndex < photos.length - 1 && (
            <button
              onClick={() => setPhotoIndex(photoIndex + 1)}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-black/40 transition-colors"
            >
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-end justify-between">
              ...
              {currentProfile.verified && (
                <div className="bg-sage-400 text-white text-xs font-medium px-2.5 py-1 rounded-lg">
                  Verified
                </div>
              )}
            </div>
        ...
        {/* Profile details below photo */}
        <div className="mt-4 space-y-4">
          {currentProfile.bio && (
            <p className="text-sage-800 text-[15px] leading-relaxed">{currentProfile.bio}</p>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {currentProfile.interests.map((interest, i) => (
                  <span
                    key={i}
                    className="bg-sage-100 text-sage-600 text-xs font-medium px-3 py-1.5 rounded-lg"
                  >
                    {interest}
                  </span>
                ))}
                ...
                {currentProfile.date_ideas.map((idea, i) => (
                  <div key={i} className="flex items-center gap-3 bg-cream-100 rounded-xl p-3">
                    ...
                    <div>
                      ...
                      {idea.location_name && (
                        <p className="text-xs text-cream-600">{idea.location_name}</p>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {/* Action buttons */}
      <div className="sticky bottom-24 md:bottom-8 pt-4 pb-2 bg-gradient-to-t from-cream-50 via-cream-50 to-transparent -mx-4 px-4">
        ...
        <p className="text-center text-xs text-cream-600 mt-2">
