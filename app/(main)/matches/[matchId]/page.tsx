...
function friendlyStatus(status: string): string {
  const map: Record<string, string> = {
    pending_pick: 'Pick a date idea', pending_accept: 'Waiting for response',
    pending_time: 'Pick a time', pending_confirm: 'Confirm time',
    confirmed: 'Date confirmed', completed: 'Date completed', rated: 'Rated', cancelled: 'Cancelled',
  };
  ...
  if (!matchInfo) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <button onClick={() => router.push('/matches')} className="text-sage-400 font-medium hover:underline">Back to messages</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col -mx-4 -my-6" style={{ height: 'calc(100vh - 8rem)' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-cream-200 bg-cream-50 shrink-0">
        <button onClick={() => router.push('/matches')} className="text-cream-700 hover:text-sage-600 p-1 -ml-1">
        ...
        <button onClick={() => showActions(matchId, matchInfo.other_user.id, matchInfo.other_user.name, () => router.push('/matches'))}
          className="text-cream-600 hover:text-sage-800 p-1">
          ...
          groupedMessages.map((group) => (
            <div key={group.date}>
              <div className="flex justify-center my-4">
                <span className="text-[11px] font-medium text-cream-600 bg-cream-200 px-3 py-1 rounded-full">{group.date}</span>
              </div>
      ...
      <div className="px-4 py-3 border-t border-cream-200 bg-cream-50 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleSend} disabled={!input.trim() || sending}
            className="w-10 h-10 bg-sage-400 rounded-full flex items-center justify-center text-white hover:bg-sage-500 disabled:opacity-40 shrink-0">
