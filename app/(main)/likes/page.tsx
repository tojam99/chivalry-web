...
export default function LikesPage() {
  ...
  const fetchLikes = useCallback(async () => {
    ...
    try {
      ...
      // Exclude matched + already swiped
      ...
      // Fetch full profiles with photos, date ideas, interests
      ...
      <div className="grid grid-cols-2 gap-3">
        {likes.map((like) => (
          <div
            ...
            onClick={() => { setSelectedProfile(like); setPhotoIndex(0); }}
          >
            {like.photos[0] ? (
              <Image src={resolvePhotoUrl(like.photos[0].photo_url)} alt={like.name} fill className="object-cover" unoptimized />
            ) : (
              <div className="w-full h-full bg-cream-300" />
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
            <div className="absolute bottom-3 left-3 right-3">
              <p className="text-white font-bold text-[15px]">{like.name}, {like.age}</p>
              {like.city && <p className="text-white/70 text-[12px]">{like.city}</p>}
            ...
            <div className="bg-cream-50 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl">
              ...
              <div className="relative aspect-[3/4] max-h-[420px] mx-5 mt-3 rounded-2xl overflow-hidden bg-cream-300">
                {selectedProfile.photos[photoIndex] && (
                  <Image
                    src={resolvePhotoUrl(selectedProfile.photos[photoIndex].photo_url, 600)}
